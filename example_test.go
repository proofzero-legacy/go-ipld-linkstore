...
func TestNewStorage(t *testing.T) {
	t.Run("Writing v1 carfile and wrapping to v2.", func(t *testing.T) {
		...
		var testLinkSystem ipld.LinkSystem

		storageLinkSystem := NewStorageLinkSystemWithNewStorage()
		...
		root := storageLinkSystem.MustStore(ipld.LinkContext{}, cidlink.LinkPrototype{}, ipld.BasicNode{})
		t.Logf("Generated CID: %v\n", root)

		//linkSystemWithNoStorage := NewStorageLinkSystemWithNewStorage()
		//Wish(t, linkSystemWithNoStorage.ReadStore,  ShouldBeSameTypeAs, readStore(testBlockReadOpener))
		//Wish(t, linkSystemWithNoStorage.WriteStore, ShouldBeSameTypeAs, writeStore(testBlockWriteOpener))
		//Wish(t, linkSystemWithNoStorage.LinkSystem, ShouldBeSameTypeAs, testLinkSystem)
	})
}
